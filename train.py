...
def main():
    ...
    parser.add_argument('--size_HR', type=int, default=512, help='size of the data crop (squared assumed)')
    ...
    model_dir = './rock_physics/save_models'
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        for i, batch in enumerate(dataloader):
            ...
            down_B = torch.nn.functional.interpolate(input=real_B,scale_factor=0.125,mode='bicubic')
            same_B = netG_A2B(down_B)
            loss_identity_B = criterion_identity(same_B, real_B)*5.0
            # G_B2A(A) should equal A if real A is fed
            # same_A = netG_B2A(netG_A2B(real_A))
            up_A = torch.nn.functional.interpolate(input=real_A,scale_factor=8,mode='bicubic')
            same_A = netG_B2A(up_A)
